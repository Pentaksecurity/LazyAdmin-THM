...
The page is just a default page, nothing interesting about it, so my next step to see if I can find any directories to work with.

      gobuster dir -w /usr/share/wordlists/dirb/common.txt -u <MACHINE_IP>

I decided to use gobuster for enumeration which gave me a few interesting results.
...
So I decided to enumerate the /content directory.
...
Which returned a lot of different results for me so I started visiting them one by one until I got to the /as directory which returned the login page.
Taking in consideration the name LazyAdmin I thought this is exactly where I needed to be and thought to research some default credentials for SweetRice.
...
SIDENOTE: ALWAYS CHANGE DEFAULT CREDENTIALS!
...
There you can put PHP code which you can later execute!

I went to https://www.revshells.com/ and used the PentestMonkey PHP shell to upload into ads.
...
We can see there is the /ads folder, which is where our shell is most likely residing.
...
Now lets create a psuedo terminal with a pty shell spawn.

      python -c 'import pty; pty.spawn("/bin/bash")'

Get to the home directory where we can see there is a directory called 'itguy'

Let's navigate there and we find our user.txt flag!
...
to see if we can perform any sudo commands.
...
backup.pl calls another file called copy.sh which is writeable by our user, so lets see what it does.

After reading the contents of the copy.sh it looks like it runs another shell.
...
Now we just need to change the IP to our machine and set up another listener and we should get a shell to root.

So here there was no nano, vim , vi or any text editor so we can use echo!
